...
logger.info(f"Today's date: {today_str_format}")
logger.info(f"Looking for data from: {yesterday_str_format}")
...
def find_most_recent_prediction_file():
    ...
    while not file_found and days_back <= MAX_DAYS_BACK:
        ...
        logger.info(f"Checking prediction file for date: {date_str}")
        filename = f"nba_games_predict_{date_str}.csv"
        file_path = os.path.join(directory_path, filename)

        if os.path.isfile(file_path):
            file_found = True
            logger.info(f"Found prediction file for {date_str}: {file_path}")
            return [file_path], date_str
        else:
            days_back += 1

    logger.warning("No prediction file found in the last %d days.", MAX_DAYS_BACK)
    return None, None
...
def find_most_recent_statistics_file():
    ...
    if file_path:
        logger.info(f"Found statistics file for {date_str}: {file_path}")
        return date_str
    else:
        logger.warning("No statistics file found within the specified range.")
        return None
...
def process_prediction_file(predict_file, last_prediction):
    ...
    """
    if not predict_file:
        logger.warning("No prediction file found to process.")
        return None
...
def update_betting_statistics(combined_df, most_recent_date):
    ...
    logger.info(f"Updating statistics using games from {most_recent_date}")
...
def main():
    ...
    if updated_df is not None:
        logger.info("Betting statistics updated successfully.")
    else:
        logger.error("Failed to update betting statistics.")
...
if __name__ == "__main__":
    try:
        main()
        logger.info("=" * 60)
        logger.info("Script 4 completed successfully")
        logger.info("=" * 60)
    except KeyboardInterrupt:
        logger.warning("Script interrupted by user")
    except Exception as e:
        logger.error("=" * 60)
        logger.error("FATAL ERROR in Script 4")
        logger.error("=" * 60)
