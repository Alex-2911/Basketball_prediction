...
def main():
    ...
    r"D:\1. Python\6. GitHub\Basketball_prediction\2026",
    "output", "Gathering_Data", "data", f"{CURRENT_SEASON}_standings"
    )
    
    monthly_file = os.path.join(STANDINGS_DIR, f"NBA_{CURRENT_SEASON}_games-{month_name}.html")

    # Delete old monthly file if exists
    if os.path.exists(monthly_file):
        try:
            os.remove(monthly_file)
            logging.info(f"Deleted outdated monthly file: {monthly_file}")
        except Exception as e:
            logging.error(f"Could not delete file {monthly_file}: {e}")
    else:
        logging.info(f"No outdated monthly file found for {month_name} ({monthly_file})")
